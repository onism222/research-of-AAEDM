...
real_delta = xlsread('Covid19.xlsx',1, 'E27 : E57'); % real daily increase number 2.21 - 3.21
real_delta = reshape(real_delta, 1, length(real_delta));

real_C = xlsread('Covid19.xlsx',1, 'G27 : G57'); % real daily cured number 2.21 - 3.21
real_C = reshape(real_C, 1, length(real_C));

n = 10; % length(real_delta) == length(real_C)
kappa_seq = zeros(1, n);
zeta_seq = zeros(1, n);
